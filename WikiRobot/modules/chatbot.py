...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Wiki AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Wiki AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        response = response.replace(
            "I'm married to my job.", "I'm married with @WikiTapiOrang"
        )
        ...
        response = response.replace(
            "Have the control right.", "@WikiTapiOrang is my owner."
        )
        response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is Wiki Nice to meet you",
...
@pbot.on_message(
    filters.regex("Wiki|wiki|robot|WIKI|Wik|wikwik|Wikwik|wik")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
